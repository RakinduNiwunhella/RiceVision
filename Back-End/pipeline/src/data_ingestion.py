...
class DataIngestorCSV(DataIngestor):
    def ingest(self, file_path_or_link, parse_dates=None):
        logger.info(f"\n{'='*60}")
        logger.info(f"DATA INGESTION - CSV")
        logger.info(f"{'='*60}")
        logger.info(f"Starting CSV data ingestion from: {file_path_or_link}")
        try:
            df = pd.read_csv(file_path_or_link, parse_dates=parse_dates)
...
class DataIngestorExcel(DataIngestor):
    def ingest(self, file_path_or_link, parse_dates=None):
        ...
        try:
            df = pd.read_excel(file_path_or_link, parse_dates=parse_dates)
...
class DataIngestorParquet(DataIngestor):
    def ingest(self, file_path_or_link: str) -> pd.DataFrame:
        logger.info("Loading parquet data from: %s", file_path_or_link)
        return pd.read_parquet(file_path_or_link)
