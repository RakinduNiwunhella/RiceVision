...
const Header = () => {
  ...
  return (
    <>
      <nav className="fixed top-4 left-1/2 -translate-x-1/2 w-[calc(100%-1.5rem)] sm:w-[calc(100%-3rem)] max-w-7xl z-50 glass h-14 rounded-2xl shadow-2xl border-white/20 overflow-visible">
        ...
        <div className="fixed inset-0 z-40 md:hidden">
          ...
          <div className="absolute top-20 left-3 right-3 glass rounded-2xl border border-white/10 shadow-2xl p-4 space-y-2 max-h-[calc(100vh-6rem)] overflow-y-auto">
            ...
            <div className="relative mb-3">
              <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-white/40 text-[18px] pointer-events-none">search</span>
              <input
                type="text"
                value={searchQuery}
            ...
            {searchQuery && filteredResults.length > 0 && (
              <div className="mb-2 rounded-xl border border-white/10 bg-white/5 overflow-hidden">
                {filteredResults.map((item) => (
                  <button
                    key={item.path}
                    onClick={() => { handleSelect(item.path); setMobileMenuOpen(false) }}
